        ...
        if (menu_active)
        {
            ...
            for (int i = 0; i < max_display_items; i++)
            {
                ...
                if (menu_index >= menu_item_count)
                    break; // Stop if we reach the end of the menu
...
{
    cc1101443jamming = true;

    display.clearDisplay();
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(10, 20);
    display.println(F("433 Spamming"));
    display.setCursor(10, 40);
    display.display();

    // buffer for TC CC data
    uint8_t ccsendingbuffer[64] = {
    ...
    while (cc1101443jamming)
    {
        ...
        for (index = 0; index < 16; index++)
        {
            ...
            ELECHOUSE_cc1101.SendData(ccsendingbuffer, sizeof(ccsendingbuffer));
        }
        vTaskDelay(10 / portTICK_PERIOD_MS); // Update every 100ms
    }
    menu_active = true;
